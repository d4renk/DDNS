...
| extra   | 扩展配置      | 对象           | `zone` 等自定义字段               | 无        | 公用配置   |
...
`extra.zone` 可用于按完整记录名显式指定 zone，Callback Provider 同样支持该能力。

- 命中映射时，优先使用 `extra.zone` 中指定的 zone
- 未命中映射时，继续使用 DDNS 当前默认的 zone 自动解析逻辑
